...
def search_wikipedia(keyword):
    ...
    try:
        ...
        search_results = response.json().get('query', {}).get('search', [])[:5]
...
def get_image_url(image_title):
    """
    Get the URL of an image from its title on Wikipedia.
    """
    API_ENDPOINT = 'https://en.wikipedia.org/w/api.php'
    params = {
        'action': 'query',
...
def main():
    ...
    json_filepath = '../data/subjects/bkds_subjects.json'
    ...
    output_filepath = f'../data/output/bkds_subjects_{timestamp}.json'
