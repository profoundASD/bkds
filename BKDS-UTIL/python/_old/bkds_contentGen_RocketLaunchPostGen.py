...
ROCKET_FILE_PATTERN = f'*{batch_id}*'
    ...
    if latest_file:
        with open(latest_file, 'r') as f:
            try:
                ...
                logMsg(f"File content preview: {json.dumps(content, indent=2)[:500]}")  # Log a preview
                # Validate and extract the latest launch if applicable
                if "result" in content and isinstance(content["result"], list):
                    latest_launch = content["result"][0]
                    logMsg(f"Extracted latest launch: {json.dumps(latest_launch, indent=2)[:500]}")
                    return latest_launch
                else:
                    logMsg("Error: JSON file does not contain the expected 'result' structure.")
                    return None
            except json.JSONDecodeError as e:
                logMsg(f"Error decoding JSON from file {latest_file}: {e}. File path: {latest_file}")
    else:
...
def generate_content_block(latest_launch):
    ...
    mission = latest_launch.get("missions", [{}])[0].get("description", "No mission description available.")
    pad = latest_launch.get("pad", {})
    pad_name = pad.get("name", "Unknown pad")
    pad_location = pad.get("location", {})
    pad_country = pad_location.get("country", "Unknown country")
    purpose = latest_launch.get("mission_description", "No mission purpose provided.")
    est_time = latest_launch.get("win_open", "Launch time not available.")
    description = latest_launch.get("launch_description", "No launch description available.")
    last_update = latest_launch.get("modified", "Last update time not available.")
    quicktext = latest_launch.get("quicktext", "")
    watch_link = extract_url(quicktext.split(' - ')[-1])  # Extract URL from quicktext
    ...
    content = (
        f"<p><strong>Mission:</strong> {mission}</p>"
        f"<p><strong>Launch Pad:</strong> {pad_name} ({pad_country})</p>"
        f"<p><strong>Purpose:</strong> {purpose}</p>"
        f"<p><strong>Estimated Launch Time:</strong> {est_time}</p>"
        f"<p><strong>Description:</strong> {description_formatted}</p>"
        f"<p><strong><a class='button-link' href='{watch_link}'>Check for video update</a></strong></p>"
        f"<p><strong>Last Update:</strong> {last_update}</p>"
    )
...
def generate_output_json(latest_launch, videos, images, related_topics):
    if not latest_launch:
        logMsg("No launch data available for output generation.")
        return []
    ...
    output = [
        {
            INSIGHT_DETAILS_KEY: {
                ...
                SUBJECT_TITLE_KEY: latest_launch.get('name', DEFAULT_VALUE),
            ...
            TEXT_BLOCK_KEY: {
                CONTENT_KEY: content,
                DESCRIPTION_KEY: f"Overview of {latest_launch.get('name', DEFAULT_VALUE)}",
            },
            MEDIA_KEY: {
                VIDEOS_KEY: videos,
                IMAGES_KEY: images,
    ...
    logMsg("Generated output JSON structure successfully.")
    return output
...
def save_output_json(output, filepath, main_feed_content_filepath):
    ...
    limited_output = {
        INSIGHT_DETAILS_KEY: {
            ...
            SUBJECT_TITLE_KEY: LAUNCH_UPDATE + new_insight_details[SUBJECT_TITLE_KEY],
    ...
    if existing_entry:
        if existing_entry != limited_output:
            ...
            logMsg("No changes detected for the existing mission. Main feed not updated.")
            return
    else:
        main_feed_content.insert(0, limited_output)
    ...
    logMsg(f"Updated main_feed_content_file with new or updated insight_details.")
