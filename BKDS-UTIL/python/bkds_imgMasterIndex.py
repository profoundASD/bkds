...
# -*- coding: utf-8 -*-
###############################################################################
# Script Name: image_processor.py
#
# Description:
#   This script processes images based on insights data and generates JSON
#   index files. It fetches data using a specified batch ID, processes image
#   URLs, handles missing images, and saves the processed data into a master
#   JSON file and category-specific JSON files. It also archives existing JSON
#   files if changes are detected, ensuring that only updated data is stored.
#
# Usage:
#   python image_processor.py <batch_id>
#
# Arguments:
#   batch_id   Unique identifier for the batch processing session.
#
# Example:
#   python image_processor.py batch_12345
#
# Requirements:
#   - Python 3.x
#   - Modules: os, re, ujson, time, random, argparse, zipfile, datetime
#     collections.defaultdict
#   - Custom modules: bkds_Utilities (providing log_msg, fetch_data, get_sqlTemplate)
#
# Author:
#   [Your Name]
#
...
output_file = os.path.join(out_dir, "images", "master_image_photos_index.json")
...
def process_images(gallery_images, featured_images, data_category, data_subject, img_dict):
    ...
    def add_or_update_image(img, label):
        ...
        if img['img_url_id'] in img_dict:
            img_dict[img['img_url_id']]['data_category'].add(sanitize_input(data_category))
            img_dict[img['img_url_id']]['data_subject'].add(sanitize_input(data_subject))
            ...
            img_dict[img['img_url_id']] = {
                ...
                'label': label,
                'data_category': {sanitize_input(data_category)},
                'data_subject': {sanitize_input(data_subject)}
            }
        written_count += 1
...
def transform_data(insights):
    ...
    # Assign data_src_index starting from 0
...
def save_to_json(data, output_file, archive_dir):
    ...
    # Sort data by data_src_index before serialization
    sorted_data = sorted(data, key=lambda x: x['data_src_index'])

    new_data_serialized = ujson.dumps(sorted_data)
...
def save_category_subject_files(images, base_path, archive_dir):
    category_dict = defaultdict(list)

    for img in images:
        for category in img['data_category']:
            sanitized_category = sanitize_input(category)
            category_dict[sanitized_category].append(img)

    # Save category files
    for category, images_list in category_dict.items():
        file_path = os.path.join(base_path, f"master_image_{category}_index.json")
        save_to_json(images_list, file_path, archive_dir)
...
def main():
    ...
    save_category_subject_files(images_data, os.path.join(out_dir, "images"), archive_dir)
