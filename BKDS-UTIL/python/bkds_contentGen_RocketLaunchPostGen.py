    ...
    output = [
        {
            INSIGHT_DETAILS_KEY: {
                ...
                SUBJECT_TITLE_KEY: latest_launch.get(TITLE_KEY, "") if latest_launch else "",
            ...
            TEXT_BLOCK_KEY: {
                CONTENT_KEY: content,
                DESCRIPTION_KEY: "Overview of " + (latest_launch.get(TITLE_KEY, "") if latest_launch else "NO DATA")
            },
            MEDIA_KEY: {
                VIDEOS_KEY: videos,
                IMAGES_KEY: images,
...
def save_output_json(output, filepath, main_feed_content_filepath):
    ...
    if existing_entry:
        if existing_entry != limited_output:
            ...
            logMsg("No changes detected for the existing mission. Data feed not updated.")
            return
    else:
        main_feed_content.insert(0, limited_output)
