...
subjGen_script="$python_path/bkds_subjGenMain.py"
alt_subjGen_script="$python_path/bkds_yt_HTMLParse.py"

shell_path="$BKDS_UTIL_SHELL"
subjGenLoad_script="$shell_path/bkds_subjGenLoad.sh"
...
supported_types="wiki, youtube (or yt), flickr"
#
################################
...
case $type in
    wiki) 
        ...
        ;;
    youtube|yt|yt_alt) 
        subprocesses["youtube"]=1
        ;;
    flickr) 
        subprocesses["flickr"]=1
        ;;
    *)
...
subjGen="python3 $subjGen_script $batch_id $type"
#
